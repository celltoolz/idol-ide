...
- **Connected catalog handlers** — when a handler is wired to one of this widget's events via the [Handlers tab](#handlers-tab) (e.g. `_set_always_on_top` → `command`), that event row shows the connected handler as a **read-only** entry (e.g. `command   _set_always_on_top`; multi-target handlers like `open_dialog` show the resolved opener, e.g. `_open_Dialog1`). The connection is managed from the Handlers tab — use its `×` / `…` buttons to disconnect or re-target — but double-clicking the row still jumps to the handler.
