...
- **GUI Designer — remaining roadmap:** font picker; anchor/justify dropdowns; tab order / z-order panel; dialog/Toplevel forms (`form_type="dialog"` slot exists); grid layout mode; live preview (run form in subprocess); persist designer sash positions; startup sash jump fix (apply h_sash width from session.json before sidebar is added to `_h_pane` to prevent layout jump on launch).
