...
| `registry.py` | `REGISTRY` dict — one entry per widget type: tk class, default size, default props, available events, `color_props`, `is_container`, `is_notebook`, `variable_prop`/`variable_types`, and a mini-preview drawing function. **Canvas widget** added as a 16th type (`tk.Canvas`, 200×150, `bg` + `image` props, click/dblclick/motion events). **Label and Button** gained `image` (file path, relative to project) and `compound` props; `_load_preview_image` renders PIL thumbnails on the design canvas using `Image.resize` (fills bounds exactly, matching runtime); button images inset 2px to match the native raised border. Non-input widgets default `"bg": ""` (OS default); input widgets default `"bg": "#FFFFFF"`. |
