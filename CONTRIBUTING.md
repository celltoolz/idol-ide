...
| `pip_manager.py` | Subprocess backend for pip install/uninstall/list — runs on daemon threads, delivers results via `after_fn`. Tracks active interpreter via `set_python(exe)`. Conda interpreters get a synthesized activation env (`utils/conda_env.runtime_env`) on every subprocess so pip works without shell activation. |
