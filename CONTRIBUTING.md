...
| `lsp_manager.py` | Protocol layer — does the `initialize` handshake, sends `textDocument/did*` notifications, handles hover/definition/diagnostics, converts paths ↔ URIs. Built on top of `LspClient`. `set_python_environment(exe)` points pylsp's jedi backend at the active interpreter (`pylsp.plugins.jedi.environment`, sent via `initializationOptions` + `workspace/didChangeConfiguration`) so completions/hovers resolve the selected venv/conda env's packages; the server itself always runs from IDOL's own env, and switching interpreters re-points the live server without a restart. |
| `git_manager.py` | Git engine — all subprocess git calls on daemon threads, fires results back via `after_fn`. Owns diff/hunk parsing, file status parsing, and the **status decoration vocabulary**: `STATUS_COLORS` / `STATUS_COLORS_LIGHT` (+ the `status_colors(kind)` picker), `GUTTER_COLORS`, `STATUS_PRIORITY`, and `folder_status(dir, status_map)` — a pure max-priority reduction that rolls everything under a folder into the one state that should colour it (`M > A/U > D`, VS Code's propagation rule). Every surface that draws M/A/U/D imports from here — explorer tree, breadcrumb file picker, tabs, source control — so the palette cannot drift; `widgets/source_control.py` re-exports `STATUS_COLORS` for its own callers rather than redefining it. **Two identity checks, sync and async:** `get_global_identity(after_fn, callback)` for anything that can draw first and fill in later, and `probe_identity() -> (ok, warning)` for callers that need the answer before they can draw at all — the New Project wizard decides from it whether to offer "Initialize git repository". `probe_identity` is **total**: every call goes through `_run_git`, which swallows a missing binary, a non-zero exit and a timeout alike, so it returns a verdict rather than raising. That is load-bearing — the wizard calls it from `__init__` on the main thread, and the three bare `subprocess.run`s it replaced (only the first of them guarded) let a slow `git config` raise `TimeoutExpired` straight through and take the dialog down. New git probes belong here, not in the widget that wants one. No UI. |
| `pip_manager.py` | Subprocess backend for pip install/uninstall/list — runs on daemon threads, delivers results via `after_fn`. Tracks active interpreter via `set_python(exe)`. Conda interpreters get a synthesized activation env (`utils/conda_env.runtime_env`) on every subprocess so pip works without shell activation. Semantic `install`/`uninstall(name, origin)` wrappers mirror `CondaManager` so the Package Manager panel treats both backends uniformly; `fetch_installed` delivers `(installed, origins)` with origins always empty for pip. `run_operation` stays public for the command palette `!pip` path. |
| `project_manager.py` | Interpreter discovery and project scaffolding — finds installed Python versions (system, venv, and conda envs via `utils/conda_env.py`; conda entries labeled `(conda: name)`), creates venvs, scaffolds starter files. `categorize_interpreter` returns `"venv"` / `"conda"` / `"system"` (pyvenv.cfg wins — a venv created from a conda python is still a venv). Daemon-threaded. |
| `conda_manager.py` | Subprocess backend for conda package operations — peer to `PipManager` with the same daemon-thread + `after_fn` contract, selected by the Package Manager panel when the active interpreter is a conda env. `fetch_installed` (`conda list -p <prefix> --json` → `(installed, origins)`; origin `"pypi"` = pip-installed), `install` (conda only — **no pip fallback by design**: conda and PyPI reuse names for different products, e.g. `graphviz`; explicit PyPI picks route through pip via the panel's source toggle), `uninstall(name, origin)` (routes to pip or `conda remove` by origin). All conda calls use `-p <prefix>` so base/named/project-local envs work identically; `available` is False when no conda exe could be located (panel then falls back to pip). Module-level ToS helpers `fetch_tos_pending` (parses `conda tos --json`; a channel with no local acceptance record — `path: "None"` — is pending) and `accept_tos` (`conda tos accept`, persisted by conda itself) power the Accept/Decline gates in the wizard and the panel via `widgets/conda_tos_dialog.py`. `_StreamCleaner` scrubs conda's TTY-oriented output inside `_stream` before lines reach the Output panel — backspace-drawn spinners collapse to their final text, ANSI cursor codes are stripped, and each download bar appears once at 100% instead of as a redraw storm. **`CondaSearchIndex`** — conda-side package search: downloads each configured channel's `channeldata.json` (channels from `utils/conda_env.configured_channels`) into `~/.idol/conda_index/`, refreshes weekly, fuzzy-searches in memory (exact > prefix > name-substring > summary); plain HTTPS, never through conda, so no ToS gate on search. |
| `script_runner.py` | Runs Python scripts as subprocesses — pushes `(line, tag)` tuples to a thread-safe queue; sends `None` sentinel on completion. Accepts `python_path` to use the active interpreter, and `cwd` for the run's working directory (None inherits IDOL's — legacy). The cwd is chosen by `app._compute_run_cwd` from the `_run_cwd_mode_var` setting ("project" = explorer root, falling back to the script dir when no project is open; "script" = the file's dir), persisted as `layout["run_cwd_mode"]`. Optional `env` overrides the subprocess environment; when None and `python_path` is a conda interpreter, `utils/conda_env.runtime_env` supplies a synthesized activation env automatically. |
| `debug_manager.py` | DAP client for debugpy — launches debugpy subprocess, connects via TCP, drives the debug session. Accepts `debugpy_site` to inject IDOL's bundled debugpy via `PYTHONPATH` (no per-project install needed), and `cwd` for the debuggee's working directory (callers pass `app._compute_run_cwd` so Run and Debug agree; falls back to the script's dir when None, never IDOL's). Note this spawns debugpy with `--listen/--wait-for-client` and sends a DAP **attach** — there is no DAP `launch` request, so cwd is set purely via `Popen`. Optional `env` overrides the debuggee environment; when None and `python_path` is a conda interpreter, a synthesized activation env is used, with the debugpy_site `PYTHONPATH` prepend layered on top either way. All callbacks dispatched via `after_fn`. |
...
| `settings.py` | **User preferences** — the store behind the Settings panel, and the module that defines what "preference" means. `SCHEMA` is a tuple of frozen `Setting` records (key, default, kind, section, label, description, optional range/choices); the panel renders from it, so adding a preference is a one-place change. `choices` may be a callable for options only known at runtime (themes are files in `themes/`). Defaults are separate from stored values — `settings.json` holds only what differs, so improving a default reaches existing users. `subscribe(cb)` drives live-apply; `reset(key)` restores a default; `migrate_legacy()` moved theme/font/minimap/Ollama-URL out of the session and show-on-startup out of `recent.json`, version-guarded and never overwriting a key the user has since set. **Scope rule, enforced here:** *preference* (follows the user) vs *workspace state* (`session.py`, follows the project) vs *project config* (`ruff.toml`/`environment.yml`, follows the code into git). Settings must **edit** project config, never mirror it. |
| `clipboard_store.py` | Clipboard-history persistence under `~/.idol/clipboard/` — one file per project root (named for a hash of the normcased absolute path), plus a shallower scratch history for when no project is open. Deliberately outside the project folder: clipboard text is arbitrary copied content, and a file in the project root is a file that gets committed. A corrupt or hand-edited history reads as empty rather than raising. `normcase` follows the platform, so case-variant paths are one history on Windows and two on POSIX — which is correct on each. |
| `starter_templates.py` | The generated `main.py` for a new GUI project, plus the `# Generated by IDOL Designer` marker and its detector. Shared because both `widgets/project_wizard.py` (project creation) and `app.py`'s Set as Main Form write that same file; as two literals they drifted, and re-pointing the main form silently rewrote it with different whitespace. Lives in `utils/` because that is what lets both reach it — `widgets/` may import `utils/`, never `app.py`. |
| `session.py` | **Workspace state** — what you were doing in *this project*, as opposed to how you like your IDE (that is `settings.py`). Saves/restores open tabs (including split-pane tabs with dirty/temp-file state), layout, breakpoints, the run configuration and the active bottom-panel tab. The `appearance` block is now empty and kept only so older files load: theme, editor font and minimap were preferences filed here, and because the same data is written into each project's `.idol-project`, restoring it meant **opening a project silently changed your theme and font**. Also saves active interpreter, and active venv or conda env (re-activates in terminal on next launch; conda envs persist the prefix dir as `interpreter["conda_prefix"]` — a separate key, since `venv_activate` restore isfile-checks an activate script — and only project-local envs under the restored root re-activate). Auto-session writes to `~/.idol/session.json`; named saves write to `.idol-project` in the project root. **Named project saves are portable**: `_rel`/`_abs` store every path *inside* the project folder as a POSIX path relative to it, so the folder can be moved or copied and still open; paths outside it (system interpreter, `~/.idol/tmp` scratch files, a tab opened from elsewhere) stay absolute, and only true descendants are relativized — a `../..` chain would break at a different nesting depth. `os.path.isabs()` is the format discriminator, so no version field is needed and all-absolute files from older versions still load. `explorer_root` is written as `"."` and **ignored** on restore: a project file's root is always the folder the file itself lives in, which is what makes a copied project correct before any repair logic runs. The auto-session passes `base=None` and stays absolute throughout (machine-global, no meaningful base). `_tab_entry` is shared by the main and split notebooks — do not re-inline it, or a path ends up relativized in one pane and not the other. **Legacy migration:** an absolute `explorer_root` marks a pre-portable file; if it also disagrees with where the file actually sits, `_remap_moved_project` re-points every path under the old root, and either way `_portable_copy` rewrites the file in the portable format so this runs once per project, not on every open. The rewrite is a pure JSON transform, deliberately *not* a `save()` call — at open time the live app state is still settling (layout stages, designer load) and would serialise a half-restored session. `_rel`/`_under` return early on a relative input so both passes are idempotent (`abspath()` would otherwise resolve it against the process CWD). **`restore()` must never leave the notebook empty** — tabs are skipped individually when their file is gone, so a session whose files all moved restores nothing while still returning `True`, and callers only run their own fallback on `False`; `_seed_empty_notebook` backstops it with the Welcome/Untitled tab a cold start would open.  **Scratch-file index** — `~/.idol/tmp/index.json` maps each `idol_tmp_<uuid>` scratch file to the tab it came from (`title`, `filepath`, `saved`), written by `_tab_entry` alongside the scratch file itself. Without it a recovered buffer could only be offered to the user as a uuid. `list_temp_files(exclude=...)` returns the union of what is on disk and what the index knows (disk wins, so an unindexed scratch file is still offered) newest-first, pruning index entries whose file is gone; `forget_temp_file(path)` deletes both. The Welcome tab's UNSAVED FILES section is the only consumer. |
| `recent.py` | Recent projects and files — read/write helpers for `~/.idol/recent.json`. `add_project(path)` / `add_file(path)` prepend entries (max 10 each); `remove_project` / `remove_file` delete by path. `get_show_on_startup()` / `set_show_on_startup(bool)` are **deprecated delegating shims** — that value is a preference and moved to `settings.py` as `general.show_welcome_on_startup`; the shims exist so no caller reads a stale copy out of `recent.json` after the migration. |
...
| `conda_env.py` | Conda environment helpers — pure path probing and env-dict construction, no subprocess. `is_conda_env`/`conda_prefix_for` (detect via `conda-meta/` at the env root — Windows python.exe sits at the root, POSIX under `bin/`), `python_exe_for`, `env_name_for` (base/named/`-p` envs), `list_conda_env_prefixes` (`~/.conda/environments.txt`, stale entries filtered), `find_base_prefixes`/`find_base_for`/`find_conda_exe` (`$CONDA_EXE`, PATH, well-known install dirs), `build_env`/`runtime_env` (synthesized activation env: PATH prepends incl. `Library\bin` + `CONDA_PREFIX`/`CONDA_DEFAULT_ENV` — fixes the Windows DLL problem without shell activation; `runtime_env` returns None for non-conda interpreters), `activation_command` (per-shell activation string sourcing the hook script explicitly — works without `conda init`; cmd unsupported, parity with venv), `configured_channels` (minimal `~/.condarc` parse, `["defaults"]` fallback) + `channeldata_urls` (channel → channeldata.json URL; `defaults` → repo.anaconda.com/pkgs/main) feeding the conda search index and `environment.yml` scaffolding. |
...
`color_picker.py`, `command_palette.py`, `completion_popup.py`, `conda_tos_dialog.py`,
`dark_checkbox.py`, `debug_panel.py`,
`explorer.py`, `find_replace.py`, `font_chooser.py`, `guide_window.py`, `learning_manager.py`,
`learning_panel.py`, `notebook.py`, `outline.py`,
`output.py`, `package_manager.py`, `problems_panel.py`, `project_wizard.py`, `references.py`,
`scrollbar.py`, `settings_panel.py`, `sidebar.py`, `source_control.py`, `statusbar.py`,
`styled_checkbox.py`, `terminal.py`, `welcome.py`

`color_picker.py` — canvas-drawn HSV picker, split into two classes on purpose. `ColorPicker` is a plain `tk.Frame` (SV square, hue strip, swatch, hex field, optional R/G/B row) that knows nothing about editors or hovering; `ColorPickerPopup` wraps one in an `overrideredirect` Toplevel and owns the hover lifetime — stay while the pointer is over the popup *or* its anchor, close after a grace delay once neither. That split is what lets the editor's inline swatch and the modal `ColorChooserDialog` share one picker. Gradients are generated at 64px and resized up, so a hue drag costs a few hundred pixels of work rather than tens of thousands. `askcolor()` is a drop-in for `tkinter.colorchooser.askcolor` — same return shape, so call sites changed only their import. **No alpha:** IDOL emits Tk colour strings and Tk has no notion of one.

`font_chooser.py` — replaces the `tkfontchooser` dependency. Family list + filter, style, size, opt-in effects (`show_effects`; underline/strikeout are Designer widget-font concerns, so the editor's caller hides them rather than showing controls it ignores), and a preview in a fixed box with `grid_propagate(False)` — without that the autohide scrollbars add their own thickness the moment they appear and a 72pt sample stretches the dialog. Font discovery is `tkinter.font.families()`, which is all tkfontchooser used and is the cross-platform answer; Windows' `@Name` rotated CJK variants are filtered out. The initial font is an **explicit argument**: the old `askfont(master, title=…, font=init)` put the dict into `**font_args` where nothing read it, so neither IDOL call site ever opened on the current font.

`dark_checkbox.py` — canvas-drawn checkbox (12px box, blue fill + ✓ when checked). `tk.Checkbutton` takes its colours from the platform and renders as a light control on a dark panel with no reliable way to restyle the indicator, so IDOL never uses it. Promoted here from `designer/menu_editor.py` when the font chooser needed the same control; menu_editor keeps a thin subclass for its own palette and its `configure(state=…)` shim.

`settings_panel.py` — the Settings tab. Two panes (categories, settings) plus a search box that spans every category and matches the dotted key as well as the label. **Renders entirely from `utils/settings.SCHEMA`** by `kind` — it knows nothing about what any individual setting means, which is what keeps adding a preference a one-line change. Writes go through `settings.set`, and the panel also *subscribes*, so a value changed from the View menu updates the row. A value change repaints one row rather than re-rendering: rows keep a fixed-width reset label that is blanked when the setting is default (the Welcome tab's marker-gutter trick), so nothing shifts and nothing flickers.
...
| File | Role |
|---|---|
| `constants.py` | Shared editing constants — auto-pair table (`_PAIRS`, `_CLOSERS`, `_SKIP_OVER` — every char that steps over an identical char already at the cursor; openers are in it too, so typing `(` before an existing `(` moves past it instead of wedging an empty pair in front), bracket-match sets (`_BRACKET_OPEN_TO_CLOSE`, `_BRACKET_CLOSE_TO_OPEN`, `_ALL_BRACKETS`), quote sets (`_QUOTES`, `_MATCH_CHARS`), editor font (`_FONT_FAMILY`, `_FONT_SIZE`), minimap width (`_MINIMAP_W`), the gutter palette (`_BREAKPOINT_COLOR`, `_BREAKPOINT_GHOST_COLOR`, `_GIT_HUNK_COLORS`), and the shared fold vocabulary — the marker regexes (`_SECTION_MARKER`, `_IDOL_BEGIN_RE`, `_IDOL_END_RE`) plus the `iter_visible(lines, folded)` generator (the single source of truth for the fold-skip walk; `FoldMixin._visual_row_of`/`_visual_to_physical`/`_visual_row_count` are thin adapters over it). Imports nothing internal — the safe leaf every module can import with no circular-import risk. |
| `tokenizer.py` | `TokenizerMixin` — regex-rule syntax highlighting: the `PYTHON_RULES` table, comment/string scanners, per-line triple-quote state scan. Reads `self.language` to pick the diff vs. python path. Also owns the **caret-context scan** the rest of the editor asks "is this position code?": `_caret_contexts(row)` returns one entry per caret slot (`len(line) + 1`, so the end-of-line slot is included) valued "code" / "string" / "comment", seeded from `_ml_state` so a docstring body is recognised on every line and not just the one that opened it; `_context_at(row, col)` is the single-position convenience. A *character* is classified from the pair straddling it — `out[i] != out[i+1]` means char *i* opened or closed something, which is what tells a string **delimiter** apart from a quote in some other string's body (the `'` in `"don't"`) and subsumes backslash-escape handling. Both the auto-pair gate and the match highlighter consume it. |
...
| `multicursor.py` | `MultiCursorMixin` — Alt+Click secondary cursors; mirrors the primary editing ops (insert / delete / newline / tab / bracket-pair / move) onto every secondary cursor, applied bottom-to-top. State (`_mc_cursors: list[tuple[int,int]]`, `_mc_anchors: list[tuple[int,int]|None]`) is host-owned; `mc_count()` public helper. `_mc_insert_char` carries every gate the primary path does — the smart-pairs preference (which it silently ignored until the pairing-context work), plain-text files, and the code-only rule. |
| `bracket_matcher.py` | `BracketMatcherMixin` — bracket + quote match highlighting. Brackets matched by a directional depth scan; quotes by a same-line parity scan over the line's string *delimiters* (opener == closer defeats depth counting). **Code-only:** the char under the cursor must have "code" context, so a bracket typed into a comment or sitting in a string's body no longer outlines against real code elsewhere; quotes are held to the matching rule instead — a quote highlights only when it genuinely delimits a string, which also fixed `'''`/`"""` pairing outer-to-outer rather than to its own neighbour. Reads `self.lines` / `cur_line` / `cur_col`, the constants, and the host's `_caret_contexts` (TokenizerMixin); render calls `_find_bracket_pair()` once per paint and outlines the returned pair. Known limitation, deliberate: the cross-line depth **scan** is still comment-unaware, because classifying every line it walks costs the whole file per paint when a bracket is unmatched. |
...
**Scoped per project** — `utils/clipboard_store.py` persists the ring, keyed by project root, and
`app._sync_clip_scope` swaps histories when the open project changes (flushing the outgoing one
first, never merging). No project open gets a shallower scratch history. The panel owns no file
I/O: it exposes `set_max`/`export_entries`/`load_entries` and an `on_change` hook, and app.py
debounces the save. `_trim` preserves recency — the older `pinned + unpinned[:keep]` rebuild
sorted every pin to the top the moment the ring first overflowed.

`welcome.py` — Welcome tab panel shown on first launch and whenever the main notebook is otherwise empty. Sections: **Start** (new file / open file / open folder / new project / open project action links), **Explore** (Learning Mode / GUI Designer / Package Manager / Settings), **What's New** (live `CHANGELOG.md` viewer with ‹ › section navigation, syntax-styled content, isolated mousewheel scroll), **Unsaved Files** (unsaved buffers with no open tab, from `session.list_temp_files`; click reopens as a dirty tab targeting the original file, × discards after confirmation; scratch files backing a currently-open tab are filtered out via the `get_open_temps` callback), **Recent Projects** and **Recent Files** (from `utils/recent.py`; click to open, × to remove; an entry whose target is gone gets a red `⊗` in the row's fixed-width marker gutter — present but blank on healthy rows so nothing shifts out of line — a dimmed name, and a `· not found` suffix, and clicking it opens `_confirm_forget` rather than silently dropping the row), rotating **Tips** (8-second cycle), and a **Show on startup** checkbox (a `DarkCheckbox` writing `general.show_welcome_on_startup` — the same key the Settings panel shows; it lived in `~/.idol/recent.json` until the settings work moved it). Global `<Enter>`/`<Leave>` activate `bind_all` wheel scrolling for the outer canvas; `_cl_text` returns `"break"` from its handler so the changelog box scrolls independently. `WelcomePanel` is constructed with nine `on_*` callbacks plus `get_open_temps`, wired in `app.py`'s `_build_welcome_tab(nb)` — one of the four panel-tab builders, so Welcome can be built into either notebook (see the panel-tab entry under Current Feature State). The three right-column lists live in `_ScrollList`, a fixed-viewport row container (`_ROWS_IN_VIEW` = 5) with a `widgets/scrollbar.VerticalScrollbar` shown only on overflow; `sync()` sizes the viewport from the rendered rows and must be called after every repopulate. The page's own scrollbar and the changelog box's are `VerticalScrollbar` too — `welcome.py` no longer imports `ttk` at all. Every one of them is constructed with `height=1` — `VerticalScrollbar` is a `tk.Canvas`, whose default requested height is 7c (~265 px), and left alone it out-requests the viewport and silently becomes what sizes the list. `_parse_changelog(path)` splits `CHANGELOG.md` on `## ` headings into `{title, lines}` dicts; `_cl_render()` inserts them into the text widget with `h3`/`bullet`/`dim` tags.
...
- **No codegen confirmation.** Code is regenerated silently — on the 0.6 s auto-gen debounce, on explicit `Ctrl+Shift+B`, and on Run when dirty. Event handlers, helper methods, and user `__init__` code are always preserved, so overwriting the `.py` is always safe.
...
- **Rename preservation.** Preserved event bodies are keyed by method name, so a rename would otherwise orphan the body and regenerate a blank stub. Renames (a retyped handler name, or a widget rename that propagates to its `_{id}_{event}` handlers) are recorded in **per-form** maps (`_pending_body_renames` / `_pending_attr_renames` in `app.py`, keyed by form name so a default id reused across forms can't cross-contaminate) and applied during the regen extraction pass: bodies/signatures are re-keyed to the new method name, and `self.<old_id>` references in user code are token-rewritten to `self.<new_id>` (`persistence.rename_self_attributes`). `_record_rename` collapses chains (A→B→C). A widget rename also repoints **Image-component canvas references** via `_propagate_widget_rename_to_components`: the `parent` canvas_ref prop and every `canvas_buttons[].canvas_id` that named the old id. An orphaned `parent` would otherwise make the CI image-component sync (`_sync_ci_image_component`) treat the canvas's image items as uncovered and regenerate a phantom `{canvas}_ci` Image component on every load — one that reappears after the user deletes it. The name editor blocks a rename that would shadow another generated or user attribute via `_designer_name_collision` (see `persistence.collect_self_attribute_targets`).
...
- **Font prop.** The font picker stores a widget's `font` prop as a Tk tuple — `("Family", size)` or `("Family", size, "bold italic")` (older forms may hold a legacy `"Family size style"` string). `canvas.py`'s `_widget_font(props)` resolves either shape into a canvas font tuple (returning `None` when unset so the draw falls back to the design default), and every text-bearing draw function passes it to `create_text`/`_text`, so font changes render live on the design surface — WYSIWYG with the codegen output (which emits the same tuple via `_font_tuple_literal`).
...
- **Interpreter statusbar segment** — shows active Python version; click to open interpreter picker popup; selection persists per project root in `~/.idol/settings.json`; venv activation (from terminal toolbar or project wizard) shown as `(.venv) Python x.x.x` and re-activated automatically on next launch; conda envs shown as `(conda: name) Python x.x.x`
- **Conda environment support** — conda envs discovered (`~/.conda/environments.txt` + default install dirs) and labeled `(conda: name)` in the picker/wizard; Run/Debug/pip execute conda pythons with a synthesized activation env (`utils/conda_env.py` — PATH incl. `Library\bin` + `CONDA_PREFIX`, no `conda init` needed); Package Manager conda backend (`editor/conda_manager.py`) with source-follows-interpreter search — conda envs search the configured channels' cached `channeldata.json` index with a `conda | PyPI` toggle; conda picks install via conda only, PyPI picks via pip-in-env with a solver warning (no silent tool swapping — conda/PyPI name collisions like `graphviz` are different products), origin-routed uninstall, `· pip` badges; terminal activation/deactivation/switch with `$CONDA_PREFIX` tracking (OSC 7778 / state file) sourcing hook scripts explicitly; wizard creates project-local `.conda/` envs from conda interpreters (yellow note, version-pinned `conda create -p`); `.conda` auto-detect in `_find_project_python`; session restore re-activates project-local conda envs. Windows-verified; POSIX paths mirror the venv code but untested on real Linux/macOS
...
- **Settings panel** (`Ctrl+,` / View → Settings / Welcome) — notebook tab, two-pane, search across all categories, rendered from `utils/settings.SCHEMA`. Reset-to-default per row. Live-apply via subscription; the View menu's remaining toggles write the same keys, so menu and panel are two surfaces on one value. Preferences moved out of the session, which used to mean opening a project could silently change your theme and editor font
- **Font chooser** (`widgets/font_chooser.py`, `Ctrl+L`) and **colour chooser** (`widgets/color_picker.py`) are IDOL's own — `tkfontchooser` dropped as a dependency and `tkinter.colorchooser` replaced in IDOL's UI. Generated user projects still emit the stdlib chooser, which is correct: their code should not depend on IDOL's widgets
- **Inline colour picker** — hovering a hex literal's swatch opens the picker; edits apply live to the document and the whole session is one undo step (via `begin_undo_group`). Quote style and hex case are preserved, `#rgb` expands. Dismissed on leave, scroll and tab change — the scroll binding lives on an *earlier bindtag* because the engine's wheel handlers return `"break"`, which suppresses `add="+"` bindings entirely
- **Breadcrumb bar** — path crumbs, filename crumb → directory file picker (folders first, current file pre-selected, folder drill-in, pick opens a new tab in the clicked pane, right-aligned M/A/U/D badges + rolled-up folder dots from `git_manager.folder_status`), symbol crumbs, sibling picker, locals drill-down, marquee scroll footer
...
- Split editor with scroll sync; scroll lock (hardware Scroll Lock key synced on startup); **only the focused pane draws a caret, both panes keep their current-line highlight** — `CanvasCodeView._has_focus()` gates the caret (primary + multi-cursor secondaries) and nothing else, and `_blink_cursor` skips its toggle+render entirely when unfocused so an inactive pane isn't repainting at 2 Hz to draw nothing. Focus transitions repaint via the canvas's `<FocusIn>`/`<FocusOut>` bindings, and `<FocusIn>` resets the blink phase so a click never waits up to 500 ms for its caret
- Find/Replace
- **Explorer** — rename, delete, drag/drop file/folder, new file/folder, context menus, unsaved-change guard on move; **Set as Root Directory** re-roots the tree only and **Open in Terminal** `cd`s the live terminal — the two halves are deliberately separate; git decorations (per-file M/A/U/D badge + colour, per-folder rolled-up `●`) drawn from `git_manager`'s shared palette. **`ttk.Treeview` tag gotcha:** competing tag options resolve by `tag_configure` *creation* order (earliest wins) and the item's own tag list order is ignored — so the `git_*` tags must be configured **before** `file`/`folder` in `__init__` or the git colours silently never render. Re-configuring a tag later (`apply_theme`) does not change its priority
- **Explorer root vs. terminal cwd** — two app-level helpers, and the distinction is load-bearing: `_set_explorer_root()` moves the tree only (Set as Root) and `BottomPanel.set_cwd()` records the start dir for the *next* shell; `_set_project_root()` moves both and is used only by project open / create / close (plus the `python main.py <file>` launch, which has no project or session to restore), and additionally `cd`s the live shell via `BottomPanel.cd_terminal()`. `_open_file()` touches **neither** — opening a file is not a workspace change. Never re-add an implicit `cd` to `set_cwd`, and never re-root from a file-open path
- **Everything that tears a project down goes through `_autosave_workspace()` first, and none of them prompt.** It writes the root's `.idol-project` when one exists (never creating one — a folder you merely opened is not a project) *and* refreshes `~/.idol/session.json`. Writing only the auto-session, as every caller used to, is why a closed-and-reopened project came back missing anything opened since the last explicit **Save Project**. No prompt is needed because the save is pure bookkeeping: dirty buffer content goes to `~/.idol/tmp` scratch files referenced from the same file, so nothing is lost by not asking. This is the rule `file_exit` already followed. Prompts belong on *tab* close and on Explorer file moves — decisions the user actually has to make
- **The split pane is project-scoped.** `_teardown_project` disposes it along with the main tabs, so one project's split files can't carry into the next; `utils/session.py` already saves and restores `split_tabs` per project file. Teardown passes `keep_temp_files=True` — every caller saves the session immediately before tearing down, and that save *references* the `~/.idol/tmp` scratch files for dirty tabs, so deleting them would restore the project with the unsaved work gone. Anything gating a prompt in front of teardown must count both panes (`_all_tab_ids`), or unsaved split work is discarded without ever asking
- **There is exactly one right pane — build it only via `_build_right_pane`.** Every widget reference the split pane needs (`_nb_frame_r`, `_notebook_r`, `_lock_btn`, `_split_mode_bar_spacer`) is a **single slot** on the app, so building a second pane while one exists doesn't duplicate state — it *orphans* the first: still packed in `_split_pane`, still bound to live handlers, unreachable from `self`. `_build_right_pane` is therefore idempotent (it calls `_dispose_split_pane()` first); `_dispose_split_pane()` is the silent teardown that `_close_split` also uses after its unsaved-changes prompts. Never call `_split_pane.add(...)` for a right pane from anywhere else, and never leave a widget's own event handler reading one of those slots — bind to the widget via default arg, or a destroyed pane's slot resurfaces as `TclError: invalid command name`
- **Tab registries are flat across both panes — use `_reveal_tab`.** `_files`, `_titles`, `_dirty`, `_codeviews`, and `_breadcrumbs` are keyed by tab id with no notion of which notebook owns the tab, so *any* lookup by path can return a split-pane tab. Two traps follow, and both have shipped as bugs: `self.notebook.select(<split tab>)` raises `TclError: not managed by`, and `_current_codeview` follows `_active_pane`, so revealing a tab in one pane and then navigating through `_outline_navigate` moves the caret in the other. `_reveal_tab(tab_id)` selects in `_which_notebook(tab_id)` *and* sets `_active_pane` to match; after calling it, drive the target's own codeview directly rather than routing through a `_current_codeview` helper
- **Both panes must get the same engine host hooks — `_new_tab_in` is not optional wiring.** `CanvasCodeView` deliberately owns no canonical state: the host sets `on_breakpoint_toggle`, `on_change`, `on_copy`, `on_completion_request` and friends, and the engine calls back. Several of those hooks have a *silent standalone fallback* for the preview-window case — an unset `on_breakpoint_toggle` makes the gutter toggle the engine's own private dot set, so a split-pane click drew a red dot while `app._breakpoints` (what the Debug panel, session save, and the DAP client all read) never learned about it. Nothing errors; the feature just quietly doesn't exist in that pane. The debug-gutter wiring now lives in `_wire_breakpoint_gutter(cv, tab_id, filepath)` and both tab builders call it — when you add a hook to `_new_tab`, either add it to `_new_tab_in` in the same commit or extract it the same way
- **Panel tabs belong to a pane, not to `self.notebook` — go through `_PANEL_TAB_SLOTS`.** Welcome, Packages, Learning Mode and Settings are single-instance tabs holding a panel rather than a code buffer, and each lives in whichever notebook opened it. The table pairs each one's tab-id attribute with its panel attribute; `_panel_tab_home(kind)` finds the notebook actually holding it (clearing the slots when it has gone, so callers read `None` as "build one"), `_toggle_panel_tab` does reveal/close, `_build_panel_tab` dispatches to the per-kind `_build_*_tab(nb)` builder, and `_panel_target_notebook()` decides where a new one lands. **Designer mode is why this exists:** `_enter_designer_mode` `pack_forget`s `self.notebook` so the canvas can take its slot in `_nb_frame_l`, which meant a tab added there was invisible and every one of those nav buttons read as dead. `_designer_split_notebook()` is the single answer to "the main notebook is off screen, where does this go" — the split pane, opened if it isn't up — and `_open_file` and `file_new` consult it too. Tk cannot reparent a widget, so relocating a panel tab is *close + rebuild*, which is also why none of it tries to preserve panel state; `_backfill_main_notebook()` is what keeps the main notebook from ever being left blank when one leaves it. Never add a fifth panel tab by hand-rolling a fifth toggle — the four it replaced had already drifted apart
...
- **Session persistence** — open tabs, layout, breakpoints, active interpreter, active venv; auto-session (`~/.idol/session.json`); named saves (`.idol-project` in project root). *Preferences* live separately in `~/.idol/settings.json` — see `utils/settings.py` for the scope rule
...
- **Events & Handlers** — catalog-driven Handlers tab (Available/Connected split; ⚡ wire, × disconnect, … options editor) powered entirely by `HANDLER_CATALOG` — adding a handler is one `HandlerDef` entry, zero `app.py` changes; `ComponentConnector` for widget/event/menu-item picking; `HandlerOptionsEditor` for mode variants; double-click navigation to handlers (stays in Designer mode when the split pane is already showing the form's `.py` — the designer is the left half of `_split_pane`, so the code is on screen beside the canvas and there is nothing to switch to); `command` constructor-kwarg events; form events (load/activate/deactivate/unload/resize)
...
- **Codegen guarantees** — `IDOL:BEGIN/END`, `IDOL:IMPORTS`, `IDOL:DIALOG_IMPORTS`, `IDOL:COMPONENTS` marker blocks; event bodies, helper methods, user imports, user `__init__` zones, and leading comments all survive regeneration; SHA-256 manual-edit detection; silent debounced auto-generation (0.6 s); Run-while-dirty prompt
