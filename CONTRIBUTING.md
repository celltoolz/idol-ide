...
| File | Role |
|---|---|
| `model.py` | `WidgetDescriptor`, `VariableBinding`, `MenuItemDescriptor`, `ComponentDescriptor`, and `FormModel` dataclasses — the canonical source of truth for every form. `FormModel` tracks name, title, size, `border_style`, `maximize_box`, `always_on_top`, `bg`, `form_type` ("main"/"dialog"), widget list, `menu_items` list, `linked_dialogs` list, and `components` list (non-visual component tray). `VariableBinding` holds the tkinter variable (StringVar/IntVar/DoubleVar/BooleanVar) bound to a widget. `MenuItemDescriptor` holds caption, name, indent (0 = top-level cascade, 1+ = item/submenu), enabled, visible, shortcut, `kind` ("command"/"checkbutton"/"radiobutton"), `variable` (tk variable name), `value` (radiobutton value string), and `command_handler` (optional handler name for check/radio items). `ComponentDescriptor` holds `id` (auto-numbered name, e.g. "timer1"), `type` (registry key), and `props` dict. |
...
| `widgets/designer_connector.py` | `ComponentConnector` modal Toplevel — wires a component handler to a widget event; left listbox shows all widgets in the form that have events, right listbox shows events for the selected widget (from `REGISTRY`); preview label shows `widget_id.event → self._comp_id_handler()`; Wire button calls `on_wire(widget_id, event_key)` and stores result in `widget.events[event_key]` (existing codegen/persistence handles it automatically) |
