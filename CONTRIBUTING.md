...
- **Resize scaling (decoupled from bg image).** Items are stored in the canvas's `_ci_orig_w/h` coordinate space (the dimensions captured on first CI-mode entry). Two independent scaling behaviors, neither requiring a background image:
  ...
  - *Font emission* — `_parse_font_spec(spec)` splits a `"Family size styles"` string (family may contain spaces) and `_font_tuple_literal(spec)` emits `repr(tuple(...))` so `create_text` always gets a proper font **tuple** (`('Segoe UI', 12, 'bold')`); a bare spaced-family string is parsed by Tk as a list and raises `expected integer`. This also fixes multi-word family fonts for all canvas text items, not just scalable ones.
