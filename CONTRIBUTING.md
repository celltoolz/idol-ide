...
| `component_registry.py` | `PropDef`, `ComponentHandlerDef`, `ComponentDef` frozen dataclasses + `COMPONENT_REGISTRY` dict — defines every non-visual component type: its icon, palette label, PropDef rows (key, label, kind, default, description), handler stubs (id, label, description, `has_connector` for ⚡ wiring, `default_body`), and any extra imports needed. Currently ships `Timer` only (uses `self.after()`, no imports). Helpers: `all_component_types()`, `get_component_def(type_key)`, `default_props(type_key)`. |
...
| `widgets/designer_connector.py` | `ComponentConnector` modal Toplevel — used for both form handlers and component handlers. Left listbox: all widgets with events; right listbox: events for the selected widget (from `REGISTRY`); optional primary `options` combobox (e.g. dialog name picker) and optional `secondary_options` combobox (e.g. close-mode picker); `wire_body_resolver` lambda for live preview of the generated call; preview label updates on every selection change; Wire button calls `on_wire(widget_id, event_key, option)` — caller decides what to do with the result (form handler wires store in `form.handler_wires`; component handler wires store in `widget.events[event_key]`). |
...
- **Available Components sub-section** (widget-selected) — component handlers with `has_connector=True` appear here so they can be wired directly from any widget's Handlers tab
